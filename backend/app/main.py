# =====================================================================
# AI Movie Recap - Web Backend (single-file version)
# Everything - the ffmpeg/Gemini/Groq/edge-tts pipeline AND the FastAPI
# app/endpoints - lives in this one file so it's easier to edit and
# re-upload from a phone. Functionally identical to the split
# pipeline.py + main.py version.
# =====================================================================
...
SUB_COLORS = {
    ...
    "#f59e0b": (245, 158, 11, 255),    # Amber Gold
    "#10b981": (16, 185, 129, 255),    # Emerald Green
    "#06b6d4": (6, 182, 212, 255),     # Cyan Sparkle
    "#ec4899": (236, 72, 153, 255),    # Sweet Rose Pink
}
...
def get_blur_mask_filter(current_video_label="[0:v]", y_position_percent=82, height_percent=15,
                          mask_color="#000000", opacity_percent=90):
    """Matches the original app's mask exactly: a solid, semi-transparent
    color bar drawn over the old hardcoded subtitles - not a gaussian blur."""
    y_position_percent = min(max(y_position_percent, 0), 96)
    height_percent = min(max(height_percent, 1), 100 - y_position_percent)
    opacity = min(max(opacity_percent, 0), 100) / 100.0
    hex_color = (mask_color or "#000000").lstrip("#")
    filt = (
        f"{current_video_label}drawbox="
        f"x=0:y=ih*({y_position_percent}/100):w=iw:h=ih*({height_percent}/100):"
        f"color=0x{hex_color}@{opacity:.2f}:t=fill[vid_masked]"
    )
    return filt, "[vid_masked]"
...
def render_subtitle_png(text, out_path, width, height, sub_y_percent=82, color="#ffffff", font_size=44):
...
async def run_recap_pipeline(
    ...
    mask_y_percent: int = 82,
    mask_height_percent: int = 15,
    mask_opacity_percent: int = 90,
    on_progress=None,
):
    ...
    if blur_mask:
        blur_str, blur_label = get_blur_mask_filter(
            "[0:v]", mask_y_percent, mask_height_percent, mask_color, mask_opacity_percent
        )
...
async def voices():
    return {
        "voices": [{"key": k, "name": v["name"]} for k, v in VOICE_MODES.items()],
        "sub_languages": SUB_LANGUAGES,
        "sub_colors": list(SUB_COLORS.keys()),
    }
...
async def create_job(
    ...
    mask_color: str = Form("#000000"),
    mask_y_percent: int = Form(82),
    mask_height_percent: int = Form(15),
    mask_opacity_percent: int = Form(90),
):
    ...
    asyncio.create_task(_run_job(
        ...
        mask_color, mask_y_percent, mask_height_percent, mask_opacity_percent,
    ))
...
async def _run_job(job_id, input_path, output_path, gemini_key, groq_key, voice, speed,
                    ...
                    blur_mask, mask_color="#000000", mask_y_percent=82, mask_height_percent=15,
                    mask_opacity_percent=90):
    async def on_progress(step, message, percent):
        JOBS[job_id].update(status="processing", step=step, message=message, percent=percent)

    try:
        result = await run_recap_pipeline(
            ...
            mask_height_percent=mask_height_percent,
